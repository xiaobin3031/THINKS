...
function bydb(data){
	...
	if(X.isEmpty(json.data))
		...
		for(var i=0;i<json.data.COLUMNS.length;i++){
			...
			t2._t('public void set'+c+'(')
			 ._t(type+' '+X.toLowerAtPos(c, 0,1)+'){').i++;
		...
		f.__('<fields> \n')._('</fields> \n')
		.__('<DataSource ID="'+tableName.replace('_','')+'" serverType="sql" tableName="'+tableName.toUpperCase()+'" dataFormat="json" showPrompt="false"> \n')
		._('<serverObject lookupStyle="spring" bean="'+X.toUpperAtPos(tableName.replace('_',''), 0, 1)+'Dao" />')._()
...
function ignoreColumn(){
	...
	var v = X.removeRepStr(X.getSepStr($('textarea[name="columns"]').val()));
	var v2,r,t;
...
function getView(){
	...
	._t('RPCManager.loadScreen("'+X.toUpperAtPos(tableName.replace('_',''), 0, 1)+'", new LoadScreenCallback() {')
	._t().i++;
	t._t('@Override')
...
function getDao(){
	...
	._t('Session hibernateSession = sessionFactory.getCurrentSession();')._()
	._t('DataSource ds = dsRequest.getDataSource();')
	._t('dsRequest.setDataFormat("json");')
	._t('Map tmp = ds.getOperationBinding("fetch");')
	._t('SQLUtil sqlUtil = new SQLUtil(true);')
	._t('StringBuffer sql = new StringBuffer(tmp.get("selectClause").toString() + tmp.get("tableClause").toString());')
	._t('//search condition')
	._t('//search condition over')
	._t('sql.append(tmp.get("orderClause") == null ? "" : tmp.get("orderClause"));')
	._t('//		Map map = dsRequest.getCriteria();')._()
	._t('int count = sqlUtil.getRecordCount(hibernateSession, sql.toString().replace(tmp.get("selectClause").toString(), "").replace((tmp.get("orderClause") == null ? "" : tmp.get("orderClause")).toString(), ""));')._()
	._t('String startRow = (dsRequest.getFieldValue("beginIndex")==null?"0":dsRequest.getFieldValue("beginIndex")).toString();')
	._t('String endRow = (dsRequest.getFieldValue("endIndex")==null?"0":dsRequest.getFieldValue("endIndex")).toString();')._()
	._t('Query query = sqlUtil.getQuery(hibernateSession, sql.toString());')
	._t('query.setFirstResult(Integer.parseInt(startRow));')
	._t('query.setMaxResults(50);')
	._t('List<HashMap<String,String>> object = query.list();')
	._t('DSResponse dsResponse = new DSResponse(dsRequest == null ? null:dsRequest.getDataSource());')
	._t('dsResponse.setData(object);')
	._t('dsResponse.setTotalRows(Integer.parseInt(endRow) - Integer.parseInt(startRow));')
	._t('dsResponse.setStartRow(0);')
	._t('dsResponse.setParameter("TotalRows", count);')
	._t('return dsResponse;').i--;
	t._t('}')._();
	
	t._t('public DSResponse add(DSRequest dsRequest,类名 item) throws Exception{').i++;
	t._t('log.info("procesing DMI add operation");')
	._t('DSResponse dsResponse = new DSResponse();')._()
	._t('Session hibernateSession = sessionFactory.getCurrentSession();')
	._t('hibernateSession.saveOrUpdate(item);')
	._t('dsResponse.setData(item);')
	._t('return dsResponse;').i--;
	t._t('}')._();
	...
	._t('DSResponse dsResponse = new DSResponse();')
	._t('// Serializable id = (Serializable) dsRequest.getFieldValue("LOCATIONID");   //获取ID')
	._t('Session hibernateSession = sessionFactory.getCurrentSession();')
	._t('//类名 item = (类名) hibernateSession.get(类名.class, id);// 类相关')._()
	._t('log.warn("fetched item:" + DataTools.prettyPrint(item));')
	._t('DataTools.setProperties(newValues, item);')
	._t('log.warn("Saving record:"+DataTools.prettyPrint(item));')
	._t('hibernateSession.saveOrUpdate(item);')
	._t('dsResponse.setData(item);')
	._t('return dsResponse;').i--;
	t._t('}')._();
	...
	._t('Session hibernateSession = sessionFactory.getCurrentSession();')
	._t('hibernateSession.delete(item);')._()
	._t('return item;').i--;
	t._t('}');
...
function searchUI(){
	...
	if(sui.indexOf('DynamicForm') < 0){
		...
		t2._t('<DynamicForm numCols="4" ID="mainForm" width="auto" height="100%" right="5" top="5" dataSource="'+tableName.replace('_','')+'" autoDraw="true" canSubmit="true" titleSuffix="" cellPadding="10" titleWidth="75">')
		._t('<fields>').i++;
		t2._t(t.toString()).i--;
		t2._t('</fields>')
		._t('</DynamicForm>').i--;
